...
async function fetchTopRecords(): Promise<GlobalRecord[]> {
  const { data, error } = await supabase
    .from('global_records')
    .select(
      'id, company_name, points, days, result_type, created_at, updated_at',
    )
    ...
    async (
      ...
      const { data: result, error: upsertError } = await supabase.rpc(
        'upsert_global_record',
        {
          ...
          p_days: Math.max(
            1,
            Math.min(10, Math.round(daysSurvived)),
          ),
      ...
      try {
        ...
        console.error(
          'La puntuación se guardó, pero falló la carga del ranking:',
          err,
        );
      }
