...
const resultBadge: Record<string, { label: string; color: string }> = {
  ...
  stable: {
    label: 'SÓLIDA',
    color: 'text-neutral-400 bg-neutral-800/40 border-neutral-700',
  },
  crisis: {
    label: 'CRISIS',
    ...
    <div className="rounded-2xl border border-neutral-800 bg-neutral-900/50 p-4 space-y-4">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 gap-3">
          ...
          <div className="w-full rounded-lg border border-red-900 bg-red-950/30 p-3 text-xs text-red-300 break-words">
            {error}
          </div>
        ...
        <div className="overflow-x-auto border border-neutral-800 rounded-xl">
          <table className="w-full border-collapse text-xs min-w-[340px]">
            ...
            <tbody>
              {records.map((r, i) => {
                const badge =
                  resultBadge[r.result_type] ?? resultBadge.stable;
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-2.5">
                      {isTop3 ? (
                        <Medal
                          className={`w-4 h-4 ${medalColors[i]}`}
                        />
                      ) : (
